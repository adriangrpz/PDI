...
class Interface(Frame):
    ...
    def toolbar(self):
        ...
        toolBar.add_command(label='Procesar blanco y negro',      command=self.proccessBW)
        toolBar.add_command(label='Procesar color',      command=self.proccessColor)
    ...
    def proccessBW(self):
        ...
        self.button = Button(self.top, textvariable=self.buttontext, command=lambda: self.proccessBWConfirm(x1Val, y1Val, x2Val, y2Val)).pack()

    """
    Function that asks the user for a custom text and the size of the squares.
    """
    def proccessColor(self):
        self.top = Toplevel()
        ...
        self.button = Button(self.top, textvariable=self.buttontext, command=lambda: self.proccessColorConfirm(x1Val, y1Val, x2Val, y2Val)).pack()

    """
    Function that checks the values given by the user and then applies the text filter.
    """
    def proccessBWConfirm(self, x1Val, y1Val, x2Val, y2Val):
        x1 = None
        x2 = None
        y1 = None
        y2 = None
    ...
    def proccessFilter(self):
        ...
        imgspool = [img.resize(size) for img in imgspool]
    ...
    def proccessColorConfirm(self, x1Val, y1Val, x2Val, y2Val):
        x1 = None
        x2 = None
        y1 = None
        y2 = None
        ...
        newImg = self.proccessColorFilter()
        self.update(newImg)
        self.top.destroy()

    def proccessColorFilter(self):
        newImg = self.image.original
        toModify = self.image.modified
